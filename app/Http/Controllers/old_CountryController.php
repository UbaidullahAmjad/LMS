...
class CountryController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        $addCountry                        =   new User;
        $addCountry->first_name            =   $request->fname;
        $addCountry->last_name             =   $request->lname;
        $addCountry->email                 =   $request->email;
        $addCountry->phone                 =   $request->mobile;
        $addCountry->password              =   Hash::make($request->password);
        if($request->status ==  0)
        {
        ...
        $addCountry->status                =   $selectedStatus;
        $addCountry->role_id               =   $request->role;
        $addCountry->address               =   $request->address;
        $addCountry->country_id            =   $country_id;
        $addCountry->state_id              =   $state_id;
        $addCountry->city_id               =   $city_id;
        $addCountry->zip_code              =   $request->zip_code;
        ...
        $addCountry->detail                =   $request->detail;
        $addCountry->facebook_url          =   $request->fb_url;
        $addCountry->youtube_url           =   $request->youtube_url;
        $addCountry->twitter_url           =   $request->twitter_url;
        $addCountry->linkedIn_url          =   $request->linkedin_url;
        if($addCountry->save())
        {
    ...
    public function edit($id)
    {
        ...
        return view('admin.countries.edit',compact('getUser','id'));
    ...
    {
        $updateCountry = User::where('id', $request->id);
        if( $request->hasFile('image'))
        {
            $image                      =   $request->file('image');
            $path                       =   public_path(). '/user_images';
    ...
    public function updateStatus($id)
    {
        ...
        $updateCountryStatus = User::where('id', $id);

        if($updateCountryStatus->update([
            'status'                =>   $selectedStatus
            ]))
        {
                return redirect()->back()->with('success','User updated successfully.');
